...
export const useDashboardMetricsOptimized = () => {
  ...
  const loadFreshData = async (businessId: string) => {
    const now = new Date();
    ...
    const growthRate = Math.random() * 20; // Placeholder
    const retentionRate = activeClients.length > 0 ? 85 : 0; // Placeholder

    // Gerar dados de receita otimizados
