...
import { BusinessInfoStepFixed } from "./steps/business-info/BusinessInfoStepFixed";
...
export const OnboardingFixed = () => {
  ...
  // Optimized auto-save with longer debouncing to prevent quota issues
  useEffect(() => {
    // Don't save during initial render or if method is null
    if (currentStep === -1 || onboardingMethod === null) return;
    
    // Rate limiting: only save once every 5 seconds
    const now = Date.now();
    ...
    // Set new timeout with longer delay to reduce saves
